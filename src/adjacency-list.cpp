...
bool
AdjacencyList::isAdjLsaBuildable(const ConfParameter& conf) const
{
  uint32_t nTimedOutNeigbors = 0;
  ...
  for (const Adjacent& adjacency : m_adjList) {
    ...
    else if (adjacency.getInterestTimedOutNo() >= conf.getInterestRetryNumber()) {
      nTimedOutNeigbors++;
    }
  }

  if (nTimedOutNeigbors == m_adjList.size()) {
    return true;
  }
  else {
    return false;
